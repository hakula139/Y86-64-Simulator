...
uint8_t Fetch::GetRA() {
    return (instruction_[1] >> 4) & 0xF;
}

uint8_t Fetch::GetRB() {
    return instruction_[1] & 0xF;
}
...
bool Fetch::InstructionIsValid() {
    return ValueIsInArray(
        icode_, {IHALT, INOP, IRRMOVQ, IIRMOVQ, IRMMOVQ, IMRMOVQ, IOPQ, IJXX,
                 ICALL, IRET, IPUSHQ, IPOPQ});
}

bool Fetch::NeedRegids() {
    return ValueIsInArray(
        icode_, {IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, IIRMOVQ, IRMMOVQ, IMRMOVQ});
}
